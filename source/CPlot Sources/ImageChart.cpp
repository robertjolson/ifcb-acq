...
int		CImageChart::ShowPopupMenu( CMenu *passedMenu, CWnd *window, UINT nFlags, CPoint point )
{
	...
	menuPtr->AppendMenu( MF_STRING | flag, kMenuScaleToWindow, _T("Scale to window"));
	flag = GetPopupCheckedFlag(this->m_MaintainAspectRatio);
	menuPtr->AppendMenu( MF_STRING | flag, kMenuMaintainAspectRatio, _T("Maintain aspect ratio"));
...

	{
		...
		menuPtr->AppendMenu( MF_POPUP | MF_STRING, (UINT) ctSubMenu.Detach(), _T("Color Table"));
	}
