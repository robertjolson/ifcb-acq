...
//-----------------------------------------------------------------------------
void CChart::GetOptimalRange(double *inRange, double *outRange,	int *nTicks, int *nMinor, BOOL exactRange) {
...
//-----------------------------------------------------------------------------
void CChart::GetOptimalRangePrivate(double *inRange, double *outRange, int *nTicks, int *nMinor, double margin, double wasted) {

	const double	cutPoint1 = 1.0, cutPoint2 = 1.25, cutPoint3 = sqrt(10.0);
...
//	int			rexp, minorTick = 0, flag;
	int			minorTick = 0, flag;
	double		rexp;
	long		test;

	// MC bodge to deal with special cases
	if ((inRange[0] == 0.0) && (inRange[1] == 0.0)) 		// both zero
		inRange[1] = 1.0;
	else if (inRange[1] == inRange[0]) 						// both equal
		inRange[1] *= 1.1;

//	mantissa = pow(10.0, temp - (int)temp);			-- Uggh!
//	rexp = (int)temp;

	temp = log10(fabs(inRange[1] - inRange[0]));
	rexp = (temp > 0) ? floor(temp) : ceil(temp);
	mantissa = pow(10.0, temp - rexp);
	...
	tickTest = RoundToRange(1.0 + (max - min - interval) / (0.5 * interval), 1.0);
	...
	test = (long)RoundToRange((max - min) / interval, 1.0);
	...
	outRange[0] = min;
	outRange[1] = max;
	*nTicks = (int)fabs(RoundToRange((max - min) / interval, 1.0)) - 1;
	*nMinor = minorTick + 1;
}
...
BOOL	CChart::WriteTextFile( CString pathName )
{
	...
	for( whichDataSet = 0; whichDataSet < GetDataSetCount(); whichDataSet++ )
	{
		...
		else
		{
			...
			string.Format( _T("Cannot save files of %d dimensions."), dataSet->GetNDimensions() );
