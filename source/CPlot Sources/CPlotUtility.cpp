...
CString	GetFontDescription( LOGFONT *font )
{
	...
	if( font == NULL ) return _T("");

	string.Format( _T("%d point %s"), font->lfHeight, font->lfFaceName );
	return string;
}
...
double	RoundToRange(double value, double range) {
	...
	double	quotient, m, fraction;
	bool negative;
		
	negative = value < 0.0;

	value = fabs(value);			// make it +ve

	quotient = value / range;
	m = floor(quotient);
	fraction = quotient - m;
...
void	AddFontSizePopup( CMenu *addMenu, LPCTSTR titleString, long compare, int startVal )
{
	CMenu			subMenu;
	int				i;
	CString			string;
	...
	for( i = 0; i < nFontSizes; i++ )
	{
		...
		string.Format( _T("%d"), fontSizes[i] );
