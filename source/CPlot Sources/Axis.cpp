...
void	CAxis::AddAxisPopupMenus( CMenu *menu, int startVal )
{
	...
	axisMenu.AppendMenu( MF_STRING | flag, startVal, _T("Auto range"));

	// Font sizes
	AddFontSizePopup( &axisMenu, _T("Title size"), this->m_TitleFont.lfHeight, startVal+kMenuAxisTitleSize );
	AddFontSizePopup( &axisMenu, _T("Label size"), this->m_LabelFont.lfHeight, startVal+kMenuAxisLabelSize );

	axisMenu.AppendMenu( MF_SEPARATOR);

	// Number of major and minor ticks
	AddNumberOfTicksPopup( &axisMenu, _T("Major ticks"), m_TickCount+1, startVal + kMenuAxisMajorTickCount );
	AddNumberOfTicksPopup( &axisMenu, _T("Minor ticks"), m_MinorTickCount, startVal + kMenuAxisMinorTickCount );

	// Add it to main menu
	switch( GetLocation() )
	{
	case kLocationLeft: string = "Left Axis"; break;
	case kLocationRight: string = "Right Axis"; break;
	case kLocationTop: string = "Top Axis"; break;
	case kLocationBottom: string = "Bottom Axis"; break;
	...
	axisMenu.AppendMenu( MF_STRING, startVal+kMenuAxisRange-1, _T("Settings..."));

	menu->AppendMenu( MF_POPUP, (UINT) axisMenu.GetSafeHmenu(), string );
}

// Adds a popup menu for the number of ticks
void	CAxis::AddNumberOfTicksPopup( CMenu *addMenu, LPCTSTR titleString, long compare, int startVal )
{
	CMenu			subMenu;
	int				i;
	CString			string;
	...
	subMenu.AppendMenu( MF_STRING | flag, startVal, _T("Auto") );

	for( i = 0; i < nTickSizes; i++ )
	{
		flag = GetPopupCheckedFlag( compare == tickSizes[i] );
		string.Format( _T("%d"), tickSizes[i] );
